...
class PostTest(unittest.TestCase):
  ...
  def test_get_posts(self):
    self.new_post.save_post()
    rcvd_posts = Post.get_specific_post(1)
    self.assertTrue(len(rcvd_posts) == 1)

  def test_get_all_posts(self):
    self.new_post.save_pitch()
    rcvd_posts = Post.get_posts()
    self.assertTrue(len(rcvd_posts) == 1)

  def test_get_post_comments(self):
    self.new_post.save_post()
    rcvd_post_comments = Post.get_comments()
    self.assertTrue(len(rcvd_post_comments)>=0)
